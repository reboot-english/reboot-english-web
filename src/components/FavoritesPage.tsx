...
import { isFound, listFavorite, lookupWord, type WordData } from '../api'
    ...
    <div className="min-h-screen px-6 py-10 sm:px-10">
      <div className="mx-auto w-full max-w-5xl">
        ...
        <header className="mb-8 flex items-baseline justify-between">
          <h1 className="font-serif text-2xl text-ink">我的收藏</h1>
          <Link
            to="/"
        ...
        <div className="flex gap-8">
          {/* left: favorite list */}
          <aside className="w-52 shrink-0 sm:w-72">
            {listStatus === 'loading' && (
            ...
            {listStatus === 'done' && words.length === 0 && (
              <p className="font-cn text-sm text-ink-faint">还没有收藏任何单词。</p>
            )}
            {listStatus === 'done' && words.length > 0 && (
          ...
          {/* right: detail panel */}
          <section className="min-w-0 flex-1 border-l border-ink/10 pl-8">
            {detailStatus === 'idle' && (
